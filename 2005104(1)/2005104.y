...
%token<symbol> LPAREN RPAREN LTHIRD RTHIRD LCURL RCURL COMMA STRUCT

%type<symbol> start program unit var_declaration declaration_list struct_declaration
...
%type<symbol> unary_expression factor term rel_expression lcurl struct_defination
...
| struct_declaration {
	$$ = new SymbolInfo("struct_declaration","unit");
	$$->startLine=$1->startLine;
	$$->endLine=$1->endLine;
	$$->child.push_back($1);
...
struct_declaration : STRUCT ID LCURL var_declaration RCURL {
	$$ = new SymbolInfo("STRUCT ID LCURL var_declaration RCURL","struct_declaration");
	$$->startLine=$1->startLine;
	$$->endLine=$5->endLine;
	$$->child.push_back($1);
	$$->child.push_back($2);
...
struct_defination : STRUCT ID declaration_list SEMICOLON {
	$$ = new SymbolInfo("STRUCT ID declaration_list SEMICOLON","struct_defination");
	$$->startLine=$1->startLine;
	$$->endLine=$4->endLine;
	$$->child.push_back($1);
	$$->child.push_back($2);
...
| struct_defination {
	$$=new SymbolInfo("struct_defination","statement");
	$$->startLine=$1->startLine;
	$$->endLine=$1->endLine;	
	$$->child.push_back($1);
	//$$->child.push_back($2);
